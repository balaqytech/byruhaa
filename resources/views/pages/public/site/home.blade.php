...
@section('content')
                ...
                <div class="relative mx-auto w-full max-w-2xl lg:mx-0">
                    <div
                        ...
                        <img src="{{ asset('images/after-twelfth-plan-workshop.png') }}"
                            alt="فتيان عُمانيون يعملون مع مرشد على خطة عملية" width="1536" height="1024" fetchpriority="high"
                            class="aspect-[3/2] h-full w-full object-cover">
                    </div>
                    <div class="mt-4 grid grid-cols-[0.72fr_1.28fr] items-end gap-4 sm:ms-12 lg:-mt-20 lg:ms-10 lg:me-[-2rem]">
                        <div
                            class="overflow-hidden rounded-sm border-4 border-[#f6fbf8] shadow-xl shadow-[#123329]/12 dark:border-[#07120f]">
        ...
        <section class="border-y border-[#2a8069]/12 bg-white/62 dark:border-white/10 dark:bg-white/[0.03]">
            <div class="mx-auto grid w-full max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[0.72fr_1.28fr] lg:px-8 lg:py-20">
                ...
                <div class="grid gap-x-8 gap-y-7 sm:grid-cols-2">
                    @foreach ([['icon' => 'book-open-text', 'title' => 'عبادة وعلم', 'body' => 'معنى واضح، وممارسة هادئة، ومساحة آمنة للسؤال.'], ['icon' => 'checkmark-badge-01', 'title' => 'عمل ومسؤولية', 'body' => 'مهمات حقيقية يتعلم فيها الفتى أن يخدم ويقرر ويتحمل.'], ['icon' => 'user-group', 'title' => 'حركة وصحبة', 'body' => 'لعب وتحديات وصحبة صالحة تصنع الألفة من دون تكلف.'], ['icon' => 'clock-01', 'title' => 'متابعة بعدية', 'body' => 'ما يبدأ في المخيم يعود مع الفتى إلى بيته ومدرسته وحياته.']] as $pillar)
    ...
    <section class="mx-auto w-full max-w-7xl px-4 py-18 sm:px-6 lg:px-8 lg:py-24">
            ...
            <article
                ...
                <div class="grid lg:grid-cols-[minmax(0,0.94fr)_minmax(0,1.06fr)] overflow-visible">
                    ...
                    <div class="order-2 flex flex-col justify-center p-7 sm:p-10 lg:order-1 lg:p-14 overflow-visible">
                        ...
                        <p class="mt-5 text-sm font-semibold text-[#87d9bd]">
                            {{ $featuredEvent->starts_at?->translatedFormat('j F Y') ?? 'يُعلن قريبًا' }}</p>
                        @if ($featuredEvent->location)
                            <p class="mt-2 text-sm text-white/80">{{ $featuredEvent->location }}</p>
                        @endif
                        <p class="mt-4 max-w-xl text-base leading-8 text-white/76">
                            {{ $featuredEvent->excerpt }}
                        </p>

                        @unless ($featuredEvent->isComingSoon())
                            ...
                            @if ($featuredTierOffer['current'])
                                <div class="mt-7 grid gap-3 sm:grid-cols-[1.15fr_0.85fr]" data-featured-tier-offer>
                                    <div class="border-s-4 border-[#87d9bd] bg-white/10 p-5">
                                        ...
                                        <p class="mt-2 font-heading text-3xl font-bold">
                                            {{ $featuredTierOffer['current']['name'] }}</p>
                                    ...
                                    @if ($featuredTierOffer['next'])
                                        <div class="bg-white/6 p-5">
                                            ...
                                            <p class="mt-2 font-heading text-2xl font-bold">
                                                {{ $featuredTierOffer['next']['name'] }}</p>
                                            <p class="mt-4 font-heading text-3xl font-bold text-[#f4dfb2]"><x-money
                                                    :amount-baisa="$featuredTierOffer['next']['price_baisa']" :currency="$featuredTierOffer['next']['currency']" /></p>
                                        </div>
                                    @endif
                                </div>
                            @elseif ($featuredTierOffer['is_sold_out'])
                                <p class="mt-7 border-s-4 border-[#f4dfb2] bg-white/8 p-5 font-heading text-2xl font-bold">
                                    اكتملت المقاعد</p>
                            @endif
                        @endunless
        ...
        @else
            <div
                class="rounded-sm border border-dashed border-[#2a8069]/24 bg-white/54 p-10 text-center dark:border-white/14 dark:bg-white/[0.03]">
                <h2 class="font-heading text-3xl font-bold text-[#123329] dark:text-[#f7f1df]">نعدّ الفعالية القادمة</h2>
                <p class="mx-auto mt-3 max-w-xl leading-8 text-[#315e52] dark:text-[#d2e7df]/72">ستظهر هنا بمجرد اعتماد
                    موعدها وفتحها للجمهور.</p>
            </div>
        @endif
    </section>
    ...
    <section
        class="mx-auto grid w-full max-w-7xl gap-10 px-4 py-18 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-center lg:px-8 lg:py-24">
        <div class="overflow-hidden rounded-sm border border-[#2a8069]/14 bg-white dark:border-white/10 dark:bg-white/5">
            <img src="{{ asset('images/after-twelfth-mentor-circle.png') }}" alt="مرشد يستمع إلى فتيين في جلسة تربوية"
                width="864" height="1821" loading="lazy"
        ...
        <div class="max-w-2xl">
            ...
            <p class="mt-5 leading-8 text-[#315e52] dark:text-[#d2e7df]/76">
                نصغي أولًا، ثم نعطي الفتى مساحة ليجرّب ويخطئ ويعيد المحاولة. المشرف هنا رفيق مسؤول، والبرنامج بيئة عملية لا
                محاضرة طويلة.
            </p>
    ...
    <section class="bg-[#0d2b25] text-white">
        <div
            class="mx-auto flex w-full max-w-7xl flex-col gap-7 px-4 py-14 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8 lg:py-16">
            <div>
                <h2 class="font-heading text-3xl font-bold lg:text-4xl">اختر التجربة التي تناسب ابنك</h2>
                <p class="mt-3 max-w-2xl leading-8 text-white/72">كل فعالية توضح عمر المشاركين وموعدها وسعرها قبل أن تبدأ
                    الحجز.</p>
            </div>
            <a href="{{ route('events.index') }}"
                class="inline-flex min-h-12 items-center justify-center gap-2 whitespace-nowrap rounded-sm bg-white px-6 py-3 text-sm font-bold text-[#0d2b25] transition hover:-translate-y-0.5 hover:bg-[#e9f7f1] active:translate-y-px motion-reduce:transition-none motion-reduce:hover:translate-y-0">
