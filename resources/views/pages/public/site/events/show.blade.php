    ...
    <article class="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <section class="grid min-h-[calc(100svh-8rem)] items-center gap-8 py-8 lg:grid-cols-[1.15fr_0.85fr]">
            <div class="public-hero-copy max-w-3xl">
                <a href="{{ route('events.index') }}"
                    class="inline-flex min-h-10 items-center gap-2 rounded-sm border border-[#009060]/18 px-3 py-2 text-sm font-bold text-[#009060] transition hover:border-[#009060]/35 hover:bg-[#009060]/8 dark:border-[#e0a800]/24 dark:text-[#e0a800] dark:hover:bg-[#e0a800]/10">
                ...
                <div class="mt-8 flex flex-wrap items-center gap-3">
                    ...
                    <span
                        class="inline-flex items-center gap-2 rounded-sm bg-[#009060]/10 px-3 py-1.5 text-xs font-semibold text-[#009060] dark:bg-[#009060]/18 dark:text-[#78ddb8]">
                ...
                <h1
                    class="mt-5 font-heading text-5xl font-bold leading-tight text-[#123329] sm:text-6xl dark:text-[#f7f1df]">
                    {{ $event->name }}
                </h1>
                ...
                <div class="mt-8 flex flex-wrap gap-3">
                    @unless ($event->isComingSoon())
                        ...
                        <a href="#event-details"
                            class="inline-flex min-h-12 items-center justify-center gap-2 rounded-sm border border-[#009060]/18 px-6 py-3 text-sm font-bold text-[#009060] transition hover:border-[#009060]/35 hover:bg-[#009060]/8 dark:border-[#e0a800]/24 dark:text-[#e0a800] dark:hover:bg-[#e0a800]/10">
            ...
            <aside
                class="public-card border border-[#009060]/12 bg-white/72 p-6 shadow-2xl shadow-[#123329]/10 dark:border-white/10 dark:bg-white/8 dark:shadow-black/30">
                <div class="flex items-center justify-between gap-5 border-b border-[#009060]/12 pb-5 dark:border-white/10">
                    <img src="{{ asset('logo.png') }}" alt="منتجع بيرحاء"
                        class="h-16 w-auto rounded-sm bg-white/70 p-2 dark:bg-white/10">
                    @unless ($event->isComingSoon())
                        <div class="text-left">
                ...
                <dl class="mt-6 grid gap-4">
                    @unless ($event->isComingSoon())
                        ...
                        <div class="grid gap-3 sm:grid-cols-2">
                            <div
                                class="rounded-sm border border-[#009060]/12 bg-white/60 p-4 dark:border-white/10 dark:bg-white/5">
                                <dt class="text-xs font-semibold text-[#123329]/54 dark:text-[#f7f1df]/54">المقاعد المتبقية</dt>
                                <dd class="mt-2 text-2xl font-bold text-[#009060] dark:text-[#e0a800]">{{ $remainingSeats }}
                                </dd>
                            </div>
                            ...
                            <div
                                ...
                                <dd class="mt-2 text-2xl font-bold text-[#123329] dark:text-[#f7f1df]">
                                    {{ $event->seat_capacity }}</dd>
                            </div>
                        </div>
                    @endunless
                    ...
                    <div class="grid gap-3 text-sm text-[#123329]/68 dark:text-[#f7f1df]/68">
                        ...
                        @if ($event->ends_at && !$event->isComingSoon())
                            <div class="flex items-center gap-3">
        ...
        @unless ($event->isComingSoon())
            <section id="event-details" class="grid gap-6 py-10 lg:grid-cols-[1.25fr_0.75fr]">
                <div class="space-y-6">
                    <div
                        class="public-card border border-[#009060]/12 bg-white/72 p-6 shadow-sm shadow-[#123329]/5 dark:border-white/10 dark:bg-white/8 dark:shadow-black/20">
                        <div class="mb-5 flex items-center gap-3">
                            <span
                                class="flex size-11 items-center justify-center rounded-sm bg-[#009060]/10 text-[#009060] dark:bg-[#e0a800]/12 dark:text-[#e0a800]">
                        ...
                        <div class="prose prose-emerald max-w-none prose-headings:font-heading prose-headings:text-[#123329] prose-p:text-[#123329]/72 prose-a:text-[#009060] prose-img:rounded-sm dark:prose-headings:text-[#f7f1df] dark:prose-p:text-[#f7f1df]/72 dark:prose-a:text-[#e0a800]"
                            dir="rtl">
                    ...
                    <div
                        ...
                        <div class="mb-5 flex items-center gap-3">
                            <span
                                class="flex size-11 items-center justify-center rounded-sm bg-[#1898b0]/10 text-[#1898b0] dark:bg-[#1898b0]/20 dark:text-[#8de7f4]">
                        ...
                        <div
                            class="mb-4 rounded-sm border border-[#009060]/18 bg-[#ecfff7] p-4 dark:border-[#e0a800]/20 dark:bg-[#009060]/10">
                            <div class="flex items-center justify-between gap-3">
                                <div>
                                    <h3 class="text-lg font-bold text-[#123329] dark:text-[#f7f1df]">
                                        {{ __('ui.payments.full_payment') }}</h3>
                                    <p class="mt-0.5 text-sm text-[#123329]/62 dark:text-[#f7f1df]/62">
                                        {{ __('ui.payments.full_payment_description') }}</p>
                                </div>
                                <x-money :amount-baisa="$event->price_baisa" :currency="$event->currency"
                                    class="font-bold text-[#009060] dark:text-[#e0a800]" />
                            </div>
                        </div>
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            @forelse ($event->paymentPlans as $paymentPlan)
                                @php
                                    ...
                                    $isValidPlan =
                                        $installments->isNotEmpty() && (int) $installments->sum('percentage') === 100;
                                    $remainingBaisa = $event->price_baisa;
                                @endphp

                                <section
                                    class="rounded-sm border border-[#009060]/12 bg-[#fbf9f1]/70 p-4 dark:border-white/10 dark:bg-white/5">
                                    <h3 class="text-lg font-bold text-[#123329] dark:text-[#f7f1df]">{{ $paymentPlan->name }}
                                    </h3>
                                    ...
                                    @if ($isValidPlan)
                                        <div class="mt-4 grid gap-3">
                                            @foreach ($installments as $installment)
                                                ...
                                                <div
                                                    class="flex items-center justify-between gap-3 rounded-sm bg-white/72 px-3 py-2 text-sm dark:bg-white/8">
                                                    <div>
                                                        <p class="font-semibold text-[#123329] dark:text-[#f7f1df]">
                                                            {{ $installment->name ?: __('ui.payments.installment_number', ['number' => $installment->sequence]) }}
                                                        </p>
                                                        <p class="mt-0.5 text-xs text-[#123329]/56 dark:text-[#f7f1df]/56">
                                                            {{ $installment->percentage }}% · <span
                                                                dir="ltr">{{ $installment->due_date?->format('Y-m-d') }}</span>
                                                        </p>
                                                    </div>
                                                    <x-money :amount-baisa="$amountBaisa" :currency="$event->currency"
                                                        class="font-bold text-[#009060] dark:text-[#e0a800]" />
                                                </div>
                                            @endforeach
                                        </div>
                                    @else
                                        <p class="mt-3 text-sm text-[#123329]/62 dark:text-[#f7f1df]/62">
                                            {{ __('ui.payments.no_plans') }}</p>
                                    @endif
                                </section>
                            @empty
                                <p
                                    class="rounded-sm border border-dashed border-[#009060]/18 p-5 text-sm text-[#123329]/62 dark:border-white/10 dark:text-[#f7f1df]/62">
                                    {{ __('ui.payments.no_plans') }}</p>
                            @endforelse
                        </div>
                    </div>
                ...
                <aside class="space-y-6">
                    <div
                        ...
                        <div class="mb-5 flex items-center gap-3">
                            <span
                                class="flex size-11 items-center justify-center rounded-sm bg-[#e0a800]/14 text-[#997300] dark:bg-[#e0a800]/18 dark:text-[#e0a800]">
                        ...
                        <div class="grid gap-3">
                            @forelse ($availableDiscounts as $discount)
                                <section
                                    ...
                                    <p class="mt-3 text-sm text-[#123329]/62 dark:text-[#f7f1df]/62">
                                        @if (
                                            $discount->minimum_family_members &&
                                                $discount->maximum_family_members &&
                                                $discount->minimum_family_members === $discount->maximum_family_members)
