...
<body class="bg-zinc-100 p-4 text-zinc-900 sm:p-8">
    ...
    <main class="mx-auto max-w-3xl rounded-xl bg-white p-6 shadow-sm sm:p-10">
        ...
        <table class="mt-6 w-full border-collapse text-sm">
            <thead><tr class="border-b border-zinc-300 text-start"><th class="p-2 text-start">الصنف</th><th class="p-2 text-start">SKU</th><th class="p-2 text-start">الكمية</th><th class="p-2 text-start">السعر شامل الضريبة</th><th class="p-2 text-start">الإجمالي</th></tr></thead>
            <tbody>
                @foreach ($order->items as $item)
                    <tr class="border-b border-zinc-100"><td class="p-2">{{ $item->product_name }} · {{ $item->option_name }}</td><td class="p-2" dir="ltr">{{ $item->sku }}</td><td class="p-2">{{ $item->quantity }}</td><td class="p-2">@if ($item->unit_discount_baisa > 0)<span class="block text-xs text-zinc-500 line-through"><x-money :amount-baisa="$item->regular_unit_price_baisa" :currency="$item->currency" /></span>@endif<x-money :amount-baisa="$item->unit_price_baisa" :currency="$item->currency" /></td><td class="p-2"><x-money :amount-baisa="$item->line_total_baisa" :currency="$item->currency" /></td></tr>
                @endforeach
            </tbody>
        </table>
