<?php

return [
    'brand' => 'Byruhaa',
    'welcome' => 'Welcome',

    'actions' => [
        'add' => 'Add',
        'back_to_login' => 'log in',
        'clear' => 'Clear',
        'complete_profile' => 'Complete profile',
        'confirm' => 'Confirm',
        'copy_link' => 'Copy link',
        'create_account' => 'Create account',
        'cancel' => 'Cancel',
        'download_contract' => 'Download contract',
        'download_pdf' => 'Download PDF',
        'edit' => 'Edit',
        'delete' => 'Delete',
        'email_password_reset_link' => 'Email password reset link',
        'log_in' => 'Log in',
        'log_out' => 'Log out',
        'manage_family' => 'Manage family',
        'open' => 'Open',
        'request_payout' => 'Request payout',
        'save' => 'Save',
        'save_password' => 'Save password',
        'sign_contract' => 'Sign contract',
        'sign_up' => 'Sign up',
        'submit_request' => 'Submit request',
        'update' => 'Update',
        'view' => 'View',
        'view_bookings' => 'View bookings',
        'view_and_sign_contract' => 'View and sign contract',
        'view_contract' => 'View contract',
        'view_events' => 'View events',
    ],

    'affiliates' => [
        'account_not_available' => 'This affiliate account is not available.',
        'available_balance' => 'Available balance',
        'code' => 'Code',
        'copied' => 'Copied',
        'dashboard_description' => 'Track referrals, commissions, and payout requests.',
        'dashboard_title' => 'Affiliate dashboard',
        'earned_total' => 'Earned total',
        'email_or_phone' => 'Email or phone number',
        'fixed_commission' => 'Fixed commission',
        'login_description' => 'Enter your email or phone number and password.',
        'login_title' => 'Affiliate login',
        'minimum_payout' => 'Minimum payout',
        'must_be_approved' => 'Your affiliate account must be approved before requesting a payout.',
        'no_commissions' => 'No commissions have been earned yet.',
        'no_payouts' => 'No payout requests yet.',
        'notes' => 'Notes',
        'paid' => 'Paid',
        'paid_referrals' => 'Paid referrals',
        'payment_details' => 'Payment details',
        'payout_amount_invalid' => 'Enter an amount between the minimum payout and your available balance.',
        'payout_created' => 'Payout request submitted.',
        'payout_request' => 'Payout request',
        'payout_request_description' => 'Request a manual payout from your available affiliate balance.',
        'payouts' => 'Payouts',
        'pending' => 'Pending',
        'pending_description' => 'Your affiliate registration is waiting for staff review.',
        'pending_title' => 'Approval pending',
        'per_booking' => 'per booking',
        'rate' => 'Rate',
        'recent_commissions' => 'Recent commissions',
        'referral_link' => 'Referral link',
        'referral_link_description' => 'Share this link to attribute public event bookings to your affiliate account.',
        'referrals' => 'Referrals',
        'register_description' => 'Create an affiliate account for staff approval.',
        'register_title' => 'Affiliate registration',
    ],

    'appearance' => [
        'dark' => 'Dark',
        'heading' => 'Appearance',
        'light' => 'Light',
        'settings' => 'Appearance settings',
        'subheading' => 'Update the appearance settings for your account',
        'system' => 'System',
    ],

    'auth' => [
        'already_have_account' => 'Already have an account?',
        'confirm_password' => 'Confirm password',
        'confirm_password_description' => 'This is a secure area of the application. Please confirm your password before continuing.',
        'create_account' => 'Create an account',
        'create_account_description' => 'Enter your details below to create your account',
        'dont_have_account' => 'Don\'t have an account?',
        'email_or_phone' => 'Email or phone number',
        'forgot_password' => 'Forgot password',
        'forgot_password_description' => 'Enter your email to receive a password reset link',
        'forgot_your_password' => 'Forgot your password?',
        'login_description' => 'Enter your email or phone number and password',
        'login_title' => 'Log in to your account',
        'or_return_to' => 'Or, return to',
        'remember_me' => 'Remember me',
        'reset_password' => 'Reset password',
        'reset_password_description' => 'Please enter your new password below',
    ],

    'bookings' => [
        'contract_pending' => 'Contract pending',
        'contract_terms' => 'Contract terms',
        'details_title' => 'Booking details',
        'empty' => 'You have not submitted any booking requests yet.',
        'family_member_count' => ':count family member(s)',
        'heading' => 'Bookings',
        'signature' => 'Signature',
        'signer_name' => 'Signer name',
        'submitted' => 'Submitted',
        'subheading' => 'Track event booking requests and contracts.',
    ],

    'blog' => [
        'all_posts' => 'All posts',
        'categories' => 'Categories',
        'empty' => 'No published posts are available right now.',
        'heading' => 'Blog',
        'published_at' => 'Published :date',
        'read_more' => 'Read more',
        'subheading' => 'Stories, updates, and useful notes from Byruhaa.',
    ],

    'dashboard' => [
        'bookings_description' => 'Track approvals, signatures, and downloadable contracts.',
        'events_description' => 'Browse published Byruhaa events.',
        'family_description' => 'Keep reusable family member profiles ready for bookings.',
        'heading' => 'Dashboard',
        'subheading' => 'Manage family profiles, event bookings, and contracts.',
    ],

    'events' => [
        'add_family_before_booking' => 'Add a family member before booking.',
        'all_family_sizes' => 'All family sizes',
        'approved_seats_remain' => 'seats available to book',
        'always_available' => 'Always available',
        'available_discounts' => 'Available discounts',
        'available_discounts_subheading' => 'Active discounts that can apply to this event.',
        'applied_discount' => 'Applied discount',
        'book_this_event' => 'Book this event',
        'checkout_subheading' => 'Select participants and review the live price before submitting.',
        'checkout_title' => 'Checkout',
        'coupon_code' => 'Coupon code',
        'coupon_code_placeholder' => 'Enter coupon code',
        'date_to_be_announced' => 'Date to be announced',
        'details_title' => 'Event details',
        'discount_per_family_member' => 'Discount per family member',
        'eligibility' => 'Eligibility',
        'eligible_age_range' => 'Eligible ages: :min to :max years',
        'empty' => 'No published events are available right now.',
        'exact_family_members' => 'For :count family members',
        'family_members' => 'Family members',
        'family_member_range' => ':min - :max family members',
        'heading' => 'Events',
        'maximum_family_members' => 'Up to :count family members',
        'minimum_family_members' => ':count+ family members',
        'open_ended' => 'Open ended',
        'outside_age_range' => 'Outside this event age range',
        'order_summary' => 'Order summary',
        'payment_plan_preview' => 'Payment plan preview',
        'payment_plan_preview_empty' => 'This plan is not ready for preview.',
        'price_per_family_member' => 'Price per family member',
        'seats_left' => 'seats left',
        'subheading' => 'Browse published trips, camps, and festivals.',
        'validity' => 'Validity',
    ],

    'errors' => [
        'label' => 'Page notice',
        'home' => 'Return home',
        'events' => 'Browse events',
        '403' => [
            'title' => 'This area is private',
            'description' => 'The page you tried to open is reserved for accounts with the right access.',
        ],
        '404' => [
            'title' => 'This path is off the map',
            'description' => 'The page may have moved, or the link no longer points to an available Byruhaa experience.',
        ],
        '419' => [
            'title' => 'Your session has refreshed',
            'description' => 'For your security, please return and try the action again from a fresh page.',
        ],
        '429' => [
            'title' => 'Too many requests',
            'description' => 'Please pause for a moment before trying again.',
        ],
        '500' => [
            'title' => 'Something needs attention',
            'description' => 'We could not complete this request right now. Please try again shortly.',
        ],
        '503' => [
            'title' => 'We will be back shortly',
            'description' => 'Byruhaa is temporarily unavailable while we prepare the experience.',
        ],
    ],

    'family' => [
        'add_family_member' => 'Add family member',
        'age' => 'Age',
        'birth_date' => 'Birth date',
        'emergency_contact' => 'Emergency contact',
        'emergency_phone' => 'Emergency phone',
        'empty' => 'No family members yet.',
        'grade' => 'Grade',
        'heading' => 'Family',
        'medical_notes' => 'Medical notes',
        'no_school_set' => 'No school set',
        'relationship_to_customer' => 'Relationship to customer',
        'school' => 'School',
        'subheading' => 'Create reusable family profiles for event bookings.',
        'edit_family_member' => 'Edit family member',
    ],

    'fields' => [
        'address' => 'Address',
        'area' => 'Area',
        'civil_id' => 'Civil ID',
        'current_password' => 'Current password',
        'email' => 'Email',
        'email_address' => 'Email address',
        'full_name' => 'Full name',
        'name' => 'Name',
        'new_password' => 'New password',
        'password' => 'Password',
        'password_confirmation' => 'Confirm password',
        'phone_number' => 'Phone number',
        'wilaya' => 'Wilaya',
    ],

    'labels' => [
        'bookings' => 'Bookings',
        'dashboard' => 'Dashboard',
        'event' => 'Event',
        'events' => 'Events',
        'family' => 'Family',
        'blog' => 'Blog',
        'payments' => 'Payments',
        'no' => 'No',
        'platform' => 'Platform',
        'settings' => 'Settings',
        'status' => 'Status',
        'yes' => 'Yes',
    ],

    'messages' => [
        'all_family_members_age_range' => 'All selected family members must be within the event age range.',
        'booking_request_submitted' => 'Booking request submitted.',
        'booking_requires_family_member' => 'A booking must include at least one family member.',
        'booking_not_found' => 'The booking could not be found.',
        'booking_must_be_approved_for_payment_plan' => 'The booking must be approved before selecting a payment plan.',
        'contract_signed' => 'Contract signed.',
        'contracts_must_be_signed_for_payment_plan' => 'All contracts must be signed before selecting a payment plan.',
        'family_member_added' => 'Family member added.',
        'family_member_deleted' => 'Family member deleted.',
        'family_member_updated' => 'Family member updated.',
        'installment_amount_too_small' => 'This installment amount cannot be paid online.',
        'installment_not_payable' => 'This installment cannot be paid right now.',
        'coupon_usage_limit_reached' => 'This coupon has reached its usage limit.',
        'invalid_coupon_code' => 'This coupon is invalid for the selected event and family members.',
        'invalid_family_member_selection' => 'One or more selected family members are invalid.',
        'invalid_signature' => 'The signature is invalid.',
        'not_enough_seats' => 'This event does not have enough remaining seats.',
        'event_capacity_below_usage' => 'Event capacity cannot be lower than held, reserved, or tier seats.',
        'event_price_below_tiers' => 'The event base price cannot be lower than an early-bird tier price.',
        'paid_booking_price_cannot_change' => 'The price of a booking with a captured payment cannot be changed.',
        'price_changed_contracts_must_be_resigned' => 'The available early-bird price changed. Updated contracts were issued; please sign them before continuing to payment.',
        'price_tier_capacity_below_usage' => 'Tier capacity cannot be lower than its held and reserved seats.',
        'price_tier_capacity_exceeded' => 'The total early-bird tier capacity cannot exceed the event capacity.',
        'price_tier_in_use' => 'A used early-bird tier cannot be deleted; deactivate it instead.',
        'price_tier_price_above_base' => 'An early-bird tier price cannot exceed the event base price.',
        'price_tier_prices_must_increase' => 'Early-bird tier prices must stay in ascending order.',
        'pay_installments_in_order' => 'Please pay the installments in order.',
        'password_updated' => 'Password updated.',
        'payment_cancelled' => 'Payment was cancelled.',
        'payment_completed' => 'Payment completed.',
        'payment_gateway_unavailable' => 'The payment gateway is unavailable. Please try again.',
        'payment_not_confirmed' => 'We could not confirm the payment yet. Please try again or contact support.',
        'payment_plan_already_selected' => 'A payment plan has already been selected for this booking.',
        'payment_plan_not_required_for_free_booking' => 'This booking has no balance due, so no payment plan is required.',
        'payment_plan_percentages_invalid' => 'The selected payment plan installments must total 100%.',
        'payment_plan_selected' => 'Payment plan selected.',
        'payment_not_refundable' => 'This payment cannot be refunded.',
        'profile_updated' => 'Profile updated.',
        'profile_incomplete' => 'Please complete your profile before continuing.',
        'profile_incomplete_alert' => 'Complete your profile to continue booking, signing contracts, managing family members, and making payments.',
        'refund_amount_invalid' => 'The refund amount is invalid.',
        'refund_gateway_unavailable' => 'The refund could not be processed. Please try again.',
        'signature_must_be_png' => 'The signature must be a PNG image.',
    ],

    'payments' => [
        'amount' => 'Amount',
        'available_after_contracts' => 'Payment plans become available after approval and all contracts are signed.',
        'choose_plan' => 'Choose a payment plan',
        'choose_plan_placeholder' => 'Select a plan',
        'complete_free_booking' => 'Complete booking',
        'completed' => 'Completed',
        'discount' => 'Discount',
        'due_date' => 'Due date',
        'empty' => 'No payment schedules are available yet.',
        'free_booking' => 'Balance covered',
        'free_booking_description' => 'Your discount covers the full booking balance. Complete the booking to mark it as paid without going to Thawani.',
        'full_payment' => 'Full payment',
        'full_payment_description' => 'Pay the full booking balance now with Thawani.',
        'heading' => 'Payments',
        'installment' => 'Installment',
        'installment_number' => 'Installment :number',
        'installment_options' => 'Installment options',
        'installment_options_description' => 'Choose an installment plan only if you do not want to pay in full.',
        'list_subheading' => 'Review your event installments, payment attempts, and refunds.',
        'no_plans' => 'No active payment plans are available for this event.',
        'no_payment_attempts' => 'No payment attempts yet.',
        'not_selected' => 'Not selected',
        'outstanding' => 'Outstanding',
        'pay_full_amount' => 'Pay full amount',
        'pay_with_thawani' => 'Pay with Thawani',
        'payment' => 'Payment',
        'payment_attempts' => 'Payment attempts',
        'plan' => 'Plan',
        'refund' => 'Refund',
        'schedule_total' => 'Schedule total',
        'select_plan' => 'Select plan',
        'subheading' => 'Choose an installment schedule after contracts are signed.',
        'subtotal' => 'Subtotal',
        'total' => 'Total',
        'total_paid' => 'Total paid',
        'total_refunded' => 'Total refunded',
        'total_scheduled' => 'Total scheduled',
        'waiting_for_previous' => 'Waiting for previous installment',
    ],

    'participant_extra' => [
        'heading' => 'Participant information',
        'select_placeholder' => 'Select an option',
        'signing_help' => 'Please complete these event-specific fields before signing.',
    ],

    'profile' => [
        'heading' => 'Profile',
        'settings' => 'Profile settings',
        'subheading' => 'Update your customer account details',
    ],

    'security' => [
        'heading' => 'Security',
        'settings' => 'Security settings',
        'subheading' => 'Update your account password',
    ],

    'settings' => [
        'heading' => 'Settings',
        'subheading' => 'Manage your profile and account settings',
    ],
];
